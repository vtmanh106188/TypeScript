"use strict";
// Show cart
function showCartItemInCart(arr) {
    ...
    `;
        document.querySelector('#shop_cart').innerHTML += item_add;
    });
}
...
// caculate Price
const tinhTien = (count, index) => {
    let price_item = Number(document.getElementsByClassName('gia')[index].innerText);
    let total = price_item * count;
    document.getElementsByClassName('tien')[index].innerText = String(total);
    tinhTongTien();
};
const tinhTongTien = () => {
    let arr_total_element = Array.from(document.getElementsByClassName('tien'));
    const arr_total = arr_total_element.map(item => Number(item.innerText));
    let total = 0;
    for (let item of arr_total) {
        total += item;
    }
    document.querySelector('#total_shopcart').innerText = String(total);
};
tinhTongTien();
// Plus and minuend
const plus = (index) => {
